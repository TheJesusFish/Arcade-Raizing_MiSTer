...
raizing_dual_ram #(.DW(64), .AW(9)) u_scr_pri_q(
    .clk0(CLK96),
    .clk1(CLK96),
    .data0  (wr_scr_q),
    .addr0  (wr_scr_q_addr),
...
wire [15:0] tile_down_offs = ({15'd0, tile_down} << 6);
wire [15:0] render_x_offs = ({14'd0, scr_x_render} << 5);
wire [15:0] scr_tile_offs = tile_down_offs + render_x_offs + excess_line;
                ...
                6: begin //rendering (2 lines for every tile)
                    if(scr_queue_i < scroll_queue_priority_n_scan_buf_i) begin
                        if(tile_num_attr > 0 ) begin
                            ...
                            SCR_TILE_NUMBER_OFFS<=scr_tile_offs;
